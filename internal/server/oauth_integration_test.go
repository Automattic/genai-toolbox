...
func setUpServerWithOAuth(t *testing.T, oauthCfg *oauth.Config, mockTools []testutils.MockTool) (chi.Router, func()) {
	t.Helper()
...
func TestMcpWithoutOAuth_NoWWWAuthenticateOnUnauthorized(t *testing.T) {
	...
	r, shutdown := setUpServerWithOAuth(t, nil, []testutils.MockTool{tool1, tool5})
	defer shutdown()
	ts := httptest.NewServer(r)
	defer ts.Close()
...
func TestOAuthFullDiscoveryFlow(t *testing.T) {
	...
	r, shutdown := setUpServerWithOAuth(t, oauthCfg, []testutils.MockTool{tool1, tool5})
	defer shutdown()
	ts := httptest.NewServer(r)
	defer ts.Close()
